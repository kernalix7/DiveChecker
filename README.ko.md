...
마우스피스에 연결된 압력 센서를 통해 입으로 불거나 빨 때의 미세한 압력 변화(-10~+200 hPa)를 **160Hz 내부 샘플링 + 설정 가능한 출력 속도(4-50Hz)**로 정밀하게 측정하고, 실시간 그래프로 시각화합니다.
...
```
[BMP280] → 160Hz → [MCU] → USB MIDI → [Flutter 앱]
    │                   │                      │
        └── 원본 센서       └── IIR + 평균화       └── 모든 로직:
            데이터             펌웨어 필터링            - 표시
                               출력: 4-50Hz            - 분석
...
**센서 스펙**
- **샘플링**: 160Hz 내부 → 4-50Hz 출력 (설정 가능)
- **펌웨어 필터링**: IIR x2 + 평균화
- **지연 시간**: ~10ms (센서 → 앱)
- **압력 범위**: -10 ~ +200 hPa (음압/양압)
- **센서 절대 범위**: 300-1250 hPa (확장)
...
| 기능 | 설명 |
|------|------|
| **듀얼코어 분리** | Core 0: USB/MIDI, Core 1: 센서 (160Hz) |
...
### ✅ v1.0.0 완료
- [x] 🎯 **실시간 압력 모니터링** - 160Hz 내부 + 설정 가능 출력
