...
<!--
각 새 버전 섹션 템플릿 — 릴리스 자를 때 이 stanza를 복사하세요.
`### Highlights`는 항상 최상단에 두세요. GitHub 릴리스 페이지에서 사용자가
가장 먼저 보는 영역이며, 릴리스 노트 스크립트가 있다면 이 섹션 전체를
그대로 가져갑니다.

### Highlights

**한 문장 헤드라인.** 필요 시 1-2 문장 부연 설명.

- 가장 중요한 사용자 가시 변경 (한 줄, 스캔 가능)
- 두 번째로 중요한 변경
- (최대 3-6 bullets, 산문 블록 금지)
