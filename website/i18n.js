...
const translations = {
    ...
    // ── DiveChecker Vent ──
    'faq.cat5': { ko: 'DiveChecker Vent', en: 'DiveChecker Vent', ja: 'DiveChecker Vent', zh: 'DiveChecker Vent', tw: 'DiveChecker Vent' },
    'faq.q20': { ko: 'DiveChecker Vent는 무엇이고, 센서와 어떻게 다른가요?', en: 'What is DiveChecker Vent, and how is it different from the DiveChecker sensor?', ja: 'DiveChecker Ventとは何で、DiveCheckerセンサーとの違いは？', zh: 'DiveChecker Vent是什么？与DiveChecker传感器有何不同？', tw: 'DiveChecker Vent是什麼？與DiveChecker感測器有何不同？' },
    'faq.a20': { ko: 'Vent는 정밀하게 교정된 풍선이 달린 <strong>마우스필 지상 트레이너</strong>입니다. 발살바나 폐에서 그냥 미는 걸로는 풍선이 부풀지 않고, <strong>충전 → T-블록 → 텅피스톤</strong> 동작이 통해야만 부풉니다. DiveChecker 센서가 압력 곡선을 측정해서 보여주는 도구라면, Vent는 동작 자체를 직접 가르쳐주는 도구예요. 함께 쓰면 학습이 완성돼요.', en: 'Vent is a <strong>dry-land Mouthfill trainer</strong> with a precisely calibrated balloon. Valsalva or simple lung exhalation won\'t inflate it — only the <strong>charge → T-block → tongue piston</strong> motion will. While the DiveChecker sensor measures and visualizes your pressure curves, Vent forces the correct motion in the first place. Used together, they close the learning loop.', ja: 'Ventは精密に校正されたバルーンを備えた<strong>マウスフィル陸上トレーナー</strong>です。バルサルバや肺からの単純な呼気では膨らまず、<strong>チャージ→Tブロック→タングピストン</strong>の動作のみで膨らみます。DiveCheckerセンサーが圧力曲線を測定・可視化するなら、Ventは動作そのものを強制的に教えるツール。併用で学習が完成します。', zh: 'Vent是配备精密校准气球的<strong>陆地Mouthfill训练器</strong>。Valsalva或简单的肺部呼气都无法吹起气球——只有<strong>charge → T-block → 舌头活塞</strong>动作才能。如果说DiveChecker传感器测量并可视化压力曲线，那么Vent则是直接强制学习正确动作的工具。合用让学习闭环完成。', tw: 'Vent是配備精密校準氣球的<strong>陸地Mouthfill訓練器</strong>。Valsalva或簡單的肺部呼氣都無法吹起氣球——只有<strong>charge → T-block → 舌頭活塞</strong>動作才能。如果說DiveChecker感測器測量並視覺化壓力曲線，那麼Vent則是直接強制學習正確動作的工具。合用讓學習閉環完成。' },
    'faq.q21': { ko: '언제부터 Vent를 쓰는 게 좋나요? 아직 프렌젤로도 잘 되는데요.', en: 'When should I start using Vent? Frenzel still works fine for me.', ja: 'Ventはいつから使い始めるのが良いですか？まだフレンゼルでうまくいっています。', zh: '什么时候开始用Vent比较好？我现在用Frenzel还行。', tw: '什麼時候開始用Vent比較好？我現在用Frenzel還行。' },
    'faq.a21': { ko: 'Vent는 <strong>딥 다이빙(보통 25~30m+)을 준비하시는 분</strong>을 위한 도구예요. 그 깊이부터는 폐가 충분히 압축되어 폐에서 짜내는 프렌젤이 더 안 통합니다. 아직 그보다 얕은 깊이만 다니신다면 DiveChecker 센서로 프렌젤·발살바 훈련만으로도 충분해요. 다음 목표가 30m+이라면 그때 Vent를 들이세요.', en: 'Vent is for <strong>freedivers preparing for deep dives (typically 25-30m+)</strong>. Past that depth, your lungs are compressed enough that Frenzel-from-the-lungs stops working. If your dives are shallower than that, the DiveChecker sensor is enough for Frenzel/Valsalva training. Pick up Vent when your next target goes past 30m.', ja: 'Ventは<strong>ディープダイビング（通常25〜30m以上）を目指す方</strong>のためのツールです。その深度を超えると肺が十分に圧縮され、肺から絞り出すフレンゼルが通用しなくなります。それより浅い深度なら、DiveCheckerセンサーでのフレンゼル・バルサルバ訓練だけで十分です。次の目標が30m以上になったらVentを導入してください。', zh: 'Vent是为<strong>准备深潜（通常25-30m以上）的潜水员</strong>设计的工具。超过那个深度后，肺部充分压缩，肺部Frenzel不再有效。如果您只下潜更浅深度，DiveChecker传感器进行Frenzel/Valsalva训练就足够了。下一个目标超过30m时再考虑Vent。', tw: 'Vent是為<strong>準備深潛（通常25-30m以上）的潛水員</strong>設計的工具。超過那個深度後，肺部充分壓縮，肺部Frenzel不再有效。如果您只下潛更淺深度，DiveChecker感測器進行Frenzel/Valsalva訓練就足夠了。下一個目標超過30m時再考慮Vent。' },
    'faq.q22': { ko: 'Vent는 DiveChecker 센서 없이도 쓸 수 있나요?', en: 'Can I use Vent without the DiveChecker sensor?', ja: 'VentはDiveCheckerセンサーなしでも使えますか？', zh: 'Vent可以不用DiveChecker传感器单独使用吗？', tw: 'Vent可以不用DiveChecker感測器單獨使用嗎？' },
    'faq.a22': { ko: '네, Vent는 단독으로 사용할 수 있어요. 풍선이 부풀면 시각적으로 성공을 확인할 수 있습니다. 다만 DiveChecker 센서를 함께 쓰면 압력 곡선으로 동작을 정량적으로 점검·교정할 수 있어, 키트로 함께 쓰시는 걸 권장합니다.', en: 'Yes, Vent works standalone — the inflated balloon is your visual feedback. Pairing it with the DiveChecker sensor adds quantitative pressure-curve analysis for finer technique correction, which is why we recommend the kit.', ja: 'はい、Vent単体で動作します。バルーンが膨らめば視覚的に成功が確認できます。ただしDiveCheckerセンサーと併用すれば圧力曲線の分析で動作を定量的に点検・矯正できるため、キットでの使用を推奨します。', zh: '可以，Vent可以独立使用——气球膨胀就是视觉反馈。但搭配DiveChecker传感器使用时，可通过压力曲线分析进行定量动作检查与矫正，因此我们推荐套装使用。', tw: '可以，Vent可以獨立使用——氣球膨脹就是視覺反饋。但搭配DiveChecker感測器使用時，可透過壓力曲線分析進行定量動作檢查與矯正，因此我們推薦套裝使用。' },
    'faq.q23': { ko: '오토벤트(Otovent)와는 어떻게 다른가요?', en: 'How is Vent different from regular Otovent?', ja: '通常のOtoventとはどう違いますか？', zh: '与普通Otovent有何不同？', tw: '與普通Otovent有何不同？' },
    'faq.a23': { ko: 'Otovent은 일반적인 이관 통기 훈련용 의료기기로, 비교적 낮은 시작 압력에 맞춰져 있습니다(귀가 단순히 펑 하는 정도). DiveChecker Vent는 <strong>마우스필 압축에 필요한 압력 수준</strong>에 맞춰 다시 교정된 풍선과 인체공학 노즈피스를 갖춘 프리다이빙 전용 트레이너예요. Otovent으로는 발살바로도 풍선이 부풀어 잘못된 습관이 강화될 수 있지만, Vent는 그게 안 됩니다.', en: 'Otovent is a medical autoinflation device with a relatively low threshold (just enough to pop your ears). DiveChecker Vent is recalibrated to <strong>Mouthfill compression pressure levels</strong>, with an ergonomic nose piece — built specifically for freediving. With Otovent the balloon inflates with Valsalva too, which can reinforce bad habits; with Vent it won\'t.', ja: 'Otoventは一般的な耳管通気訓練用の医療機器で、比較的低い始動圧力に設定されています（単に耳がポンと開く程度）。DiveChecker Ventは<strong>マウスフィル圧縮に必要な圧力水準</strong>に再校正されたバルーンと人体工学ノーズピースを備えたフリーダイビング専用トレーナーです。Otoventではバルサルバでもバルーンが膨らんでしまい誤った習慣が強化されかねませんが、Ventではそれが起きません。', zh: 'Otovent是用于一般咽鼓管通气训练的医疗器械，起始压力较低（仅够耳朵啵的一声打开）。DiveChecker Vent是按<strong>Mouthfill压缩所需压力水平</strong>重新校准的气球，配备人体工学鼻塞，是自由潜水专用训练器。Otovent用Valsalva也能吹起气球，反而可能强化错误习惯；Vent不会发生这种情况。', tw: 'Otovent是用於一般耳咽管通氣訓練的醫療器械，起始壓力較低（僅夠耳朵啵的一聲打開）。DiveChecker Vent是按<strong>Mouthfill壓縮所需壓力水準</strong>重新校準的氣球，配備人體工學鼻塞，是自由潛水專用訓練器。Otovent用Valsalva也能吹起氣球，反而可能強化錯誤習慣；Vent不會發生這種情況。' },
    ...
    'vent.hero.desc': { ko: '딥 다이빙 가서야 마우스필을 처음 시도하시나요? 교정된 풍선이 <strong>마우스필 압축에 필요한 저항</strong>을 만들어줘서, 충전·T-블록·텅피스톤 동작을 지상에서 정확히 다질 수 있어요.<br>풍선이 부풀면 그게 곧 성공 &middot; 어디서나 매일 5분', en: 'First time trying Mouthfill on a deep dive? A pressure-calibrated balloon provides <strong>the resistance Mouthfill compression needs</strong> — drill the charge, T-block, and tongue-piston motion on dry land before it matters.<br>Visual feedback by balloon size &middot; 5 minutes a day, anywhere', ja: 'ディープダイビングで初めてマウスフィルを試しますか？校正されたバルーンが<strong>マウスフィル圧縮に必要な抵抗</strong>を作り出し、チャージ・Tブロック・タングピストンの動作を陸上で正確に練習できます。<br>バルーンが膨らめば成功 &middot; どこでも1日5分', zh: '到深潜时才第一次尝试Mouthfill？校准的气球提供<strong>Mouthfill压缩所需的阻力</strong>，让您在陆地上精确练习charge、T-block、舌头活塞动作。<br>气球膨胀即为成功 &middot; 随时随地每天5分钟', tw: '到深潛時才第一次嘗試Mouthfill？校準的氣球提供<strong>Mouthfill壓縮所需的阻力</strong>，讓您在陸地上精確練習charge、T-block、舌頭活塞動作。<br>氣球膨脹即為成功 &middot; 隨時隨地每天5分鐘' },
    ...
    'vent.hero.stat2-label': { ko: '풍선 1개당', en: 'Cycles per balloon', ja: 'バルーンあたりサイクル', zh: '每个气球周期', tw: '每個氣球週期' },
    ...
    'vent.problem.1.desc': { ko: '마우스필은 보통 25~30m 이후, 폐에서 프렌젤이 더 이상 통하지 않는 깊이부터 의미가 있어요. 풀은 깊이가 부족하고, 바다 시도는 매번 본 다이브 부담이에요. 결국 본 시도에서야 처음 해보게 됩니다.', en: 'Mouthfill only matters past ~25-30m, where Frenzel from the lungs stops working. Pools are too shallow; every ocean attempt costs a full dive. So most divers end up doing it for real before they\'ve drilled it.', ja: 'マウスフィルは通常25〜30m以降、肺からのフレンゼルが通用しなくなる深度から意味があります。プールは深度不足、海での試みは毎回本ダイブの負担。結局、本ダイブで初めて試すことになります。', zh: 'Mouthfill通常在25~30m以后，肺部Frenzel不再有效的深度才有意义。泳池深度不够，海中尝试每次都伴随真实潜水的负担。结果许多潜水员到正式潜水时才第一次尝试。', tw: 'Mouthfill通常在25~30m以後，肺部Frenzel不再有效的深度才有意義。泳池深度不夠，海中嘗試每次都伴隨真實潛水的負擔。結果許多潛水員到正式潛水時才第一次嘗試。' },
    'vent.problem.2.title': { ko: '제대로 한 건지 모르겠어요', en: 'No way to know if it worked', ja: 'うまくいったか分からない', zh: '不知道是否做对了', tw: '不知道是否做對了' },
    'vent.problem.2.desc': { ko: 'T-블록이 제대로 닫혔는지, 텅피스톤이 실제로 압력을 만들어내는지 — 객관적으로 확인할 방법이 없어요. 결국 본 다이브에서 "되네" "안 되네"만 알게 됩니다.', en: 'Is your T-block sealed? Is the tongue piston actually generating pressure? No objective way to check until depth — and by then you either get the dive, or abort it.', ja: 'Tブロックが正しく閉じたか、タングピストンが実際に圧力を生み出しているか — 客観的に確認する方法がありません。結局、本ダイブで「できた」「できなかった」しか分かりません。', zh: 'T-block是否密封？舌头活塞是否真的产生压力？没有客观方法验证，直到下潜——届时只能确认"成功"或"中止"。', tw: 'T-block是否密封？舌頭活塞是否真的產生壓力？沒有客觀方法驗證，直到下潛——屆時只能確認「成功」或「中止」。' },
    ...
    // ── SOLUTION ──
    'vent.solution.title': { ko: '풍선이 부풀면<br><span class="gradient-text">마우스필이 작동</span>한다는 뜻', en: 'When the balloon inflates,<br><span class="gradient-text">your Mouthfill is working</span>', ja: 'バルーンが膨らめば<br><span class="gradient-text">マウスフィル成立</span>の証', zh: '气球膨胀<br>就是<span class="gradient-text">Mouthfill成功</span>的证明', tw: '氣球膨脹<br>就是<span class="gradient-text">Mouthfill成功</span>的證明' },
    ...
    'vent.feat1.title': { ko: '마우스필 압력에 맞춰 교정된 풍선', en: 'Calibrated to Mouthfill pressure levels', ja: 'マウスフィル圧に合わせて校正', zh: '按Mouthfill压力水平校准', tw: '按Mouthfill壓力水準校準' },
    'vent.feat1.desc': { ko: '풍선은 일정 압력 이상에서만 부풀도록 정밀하게 교정되어 있어요. 마우스필 압축에서 만들어내야 하는 압력 수준이죠.<br>발살바나 폐에서 그냥 미는 걸로는 부풀지 않습니다. <strong>충전 → T-블록 → 텅피스톤</strong> 동작이 통해야만 풍선이 커져요.', en: 'The balloon is precisely calibrated to inflate only past a threshold — the pressure your Mouthfill compression needs to generate.<br>Valsalva or simple lung exhalation won\'t do it. Only <strong>charge → T-block → tongue piston</strong> will inflate it.', ja: 'バルーンは一定圧力以上でのみ膨らむよう精密に校正されています。マウスフィル圧縮で生み出すべき圧力水準です。<br>バルサルバや肺からの単純な呼気では膨らみません。<strong>チャージ→Tブロック→タングピストン</strong>の動作が通じなければ膨らまないのです。', zh: '气球经过精密校准，只有压力超过阈值才膨胀——这正是Mouthfill压缩需要产生的压力水平。<br>Valsalva或单纯肺部呼气都不行。只有<strong>charge → T-block → 舌头活塞</strong>动作才能让它膨胀。', tw: '氣球經過精密校準，只有壓力超過閾值才膨脹——這正是Mouthfill壓縮需要產生的壓力水準。<br>Valsalva或單純肺部呼氣都不行。只有<strong>charge → T-block → 舌頭活塞</strong>動作才能讓它膨脹。' },
    'vent.feat1.tag1': { ko: '마우스필 압축', en: 'Mouthfill compression', ja: 'マウスフィル圧縮', zh: 'Mouthfill 压缩', tw: 'Mouthfill 壓縮' },
    'vent.feat1.tag2': { ko: '충분한 역압', en: 'Sufficient back-pressure', ja: '十分な逆圧', zh: '充足的反压', tw: '充足的反壓' },
    ...
    'vent.feat2.desc': { ko: '"제대로 했나?" 같은 의심이 사라집니다. 풍선이 자몽 크기로 부풀면 마우스필 압축이 충분한 압력을 만들어냈다는 뜻이에요.<br>학생도, 강사도, 옆에서 보는 사람도 같은 결과를 봐요.', en: 'No more "did it work?" Grapefruit-sized balloon means your Mouthfill compression generated the required pressure.<br>Student, instructor, and bystander see the same result.', ja: '「うまくいった？」という疑いが消えます。バルーンがグレープフルーツ大に膨らめば、マウスフィル圧縮が必要な圧力を生み出した証拠です。<br>生徒も講師も、横で見ている人も同じ結果を見ます。', zh: '"做对了吗？"的疑虑消失。气球膨胀到葡萄柚大小就意味着Mouthfill压缩产生了所需压力。<br>学生、教练、旁观者同时看到同样的结果。', tw: '「做對了嗎？」的疑慮消失。氣球膨脹到葡萄柚大小就意味著Mouthfill壓縮產生了所需壓力。<br>學生、教練、旁觀者同時看到同樣的結果。' },
    ...
    'vent.feat3.desc': { ko: '비행기 안에서도, 출장지 호텔에서도, 다이빙 가는 차 안에서도. 풀이나 바다 없이 매일 5분이면 됩니다.<br>본 다이브 가기 전, T-블록과 텅피스톤 자세를 충분히 다질 수 있어요.', en: 'On a plane, in a hotel, in your car before a dive. No pool or ocean — five minutes a day is enough.<br>Drill the T-block and tongue-piston motion as much as you need before the actual dive.', ja: '飛行機内、出張先のホテル、ダイビング前の車内。プールも海もなしで毎日5分。<br>本ダイブ前、Tブロックとタングピストンの姿勢を十分に固められます。', zh: '飞机上、出差酒店里、潜水前的车里。无需泳池或海洋——每天5分钟即可。<br>正式深潜前，充分巩固T-block与舌头活塞动作。', tw: '飛機上、出差飯店裡、潛水前的車裡。無需泳池或海洋——每天5分鐘即可。<br>正式深潛前，充分鞏固T-block與舌頭活塞動作。' },
    ...
    'vent.feat3.tag3': { ko: '매일 5분', en: '5 min daily', ja: '毎日5分', zh: '每天5分钟', tw: '每天5分鐘' },
    ...
    'vent.feat4.desc': { ko: 'Vent로는 <strong>마우스필 동작</strong>을 익히고, DiveChecker 센서로는 <strong>압력 곡선</strong>으로 검증합니다.<br>같이 쓰면 학습이 완성돼요 — 해보고, 측정하고, 고치고.', en: 'Vent trains the <strong>Mouthfill motion</strong>, the DiveChecker sensor verifies it with the <strong>pressure curve</strong>.<br>Together they close the loop: do, measure, fix.', ja: 'Ventで<strong>マウスフィル動作</strong>を身につけ、DiveCheckerセンサーで<strong>圧力曲線</strong>として検証。<br>一緒に使えば学習が完成します — やって、測って、直して。', zh: 'Vent训练<strong>Mouthfill动作</strong>，DiveChecker传感器用<strong>压力曲线</strong>验证。<br>合用闭环学习——做、测、改。', tw: 'Vent訓練<strong>Mouthfill動作</strong>，DiveChecker感測器用<strong>壓力曲線</strong>驗證。<br>合用閉環學習——做、測、改。' },
    ...
    'vent.feat4.tag3': { ko: '트레이닝 키트', en: 'Training kit', ja: 'トレーニングキット', zh: '训练套件', tw: '訓練套件' },

    // ── HOW IT WORKS ──
    'vent.how.label': { ko: 'HOW IT WORKS', en: 'HOW IT WORKS', ja: 'HOW IT WORKS', zh: 'HOW IT WORKS', tw: 'HOW IT WORKS' },
    'vent.how.title': { ko: '풍선이 마우스필을 검증하는 원리', en: 'How the balloon validates Mouthfill', ja: 'バルーンがマウスフィルを検証する仕組み', zh: '气球如何验证Mouthfill', tw: '氣球如何驗證Mouthfill' },
    'vent.how.sub': { ko: '풍선은 일정 압력 이상에서만 부풉니다. 마우스필 압축이 충분히 작동했을 때만 시각적으로 확인되는 거죠', en: 'The balloon only inflates past a threshold. Visual confirmation that your Mouthfill compression generated enough pressure', ja: 'バルーンは一定圧力以上でのみ膨らみます。マウスフィル圧縮が十分に作動した時だけ視覚的に確認できる仕組みです', zh: '气球只有压力超过阈值才膨胀。这样Mouthfill压缩成功时才能视觉确认', tw: '氣球只有壓力超過閾值才膨脹。這樣Mouthfill壓縮成功時才能視覺確認' },
    ...
    'vent.how.s2.title': { ko: '충전 → T-블록 → 텅피스톤', en: 'Charge → T-block → tongue piston', ja: 'チャージ → Tブロック → タングピストン', zh: 'Charge → T-block → 舌头活塞', tw: 'Charge → T-block → 舌頭活塞' },
    'vent.how.s2.desc': { ko: '입에 공기를 모으고(<strong>충전</strong>), 후두를 닫아 폐와 입을 분리(<strong>T-블록</strong>), 혀와 연구개로 그 공기를 코쪽으로 짜내세요(<strong>텅피스톤</strong>). 발살바로는 풍선이 부풀지 않습니다.', en: 'Take air into your mouth (<strong>charge</strong>), close off the throat to isolate from the lungs (<strong>T-block</strong>), then push that air into the nasal passage with your tongue and soft palate (<strong>tongue piston</strong>). Valsalva won\'t inflate the balloon.', ja: '口に空気を取り込み（<strong>チャージ</strong>）、喉頭を閉じて肺と口を分離し（<strong>Tブロック</strong>）、舌と軟口蓋でその空気を鼻側に押し出す（<strong>タングピストン</strong>）。バルサルバではバルーンは膨らみません。', zh: '将空气吸入口腔（<strong>charge</strong>），闭合喉部使口腔与肺部分离（<strong>T-block</strong>），用舌头和软腭将该空气推向鼻腔（<strong>舌头活塞</strong>）。Valsalva无法吹起气球。', tw: '將空氣吸入口腔（<strong>charge</strong>），閉合喉部使口腔與肺部分離（<strong>T-block</strong>），用舌頭和軟顎將該空氣推向鼻腔（<strong>舌頭活塞</strong>）。Valsalva無法吹起氣球。' },
    'vent.how.s3.title': { ko: '자몽만큼 부풀면 성공', en: 'Grapefruit size = success', ja: 'バルーンがグレープフルーツ大なら成功', zh: '气球到葡萄柚大小＝成功', tw: '氣球到葡萄柚大小＝成功' },
    'vent.how.s3.desc': { ko: '한 번에 2~3초씩, 양쪽 콧구멍을 번갈아 5세트. 매일 반복하면 텅피스톤 근육 패턴이 자리 잡혀요.', en: 'Hold each inflation for 2~3 seconds, alternate nostrils, 5 sets. Daily repetition builds the tongue-piston muscle pattern.', ja: '1回2〜3秒、左右の鼻孔を交互に5セット。毎日繰り返すとタングピストンの筋肉パターンが定着します。', zh: '每次保持2~3秒，左右鼻孔交替，5组。每天重复使舌头活塞的肌肉模式形成。', tw: '每次保持2~3秒，左右鼻孔交替，5組。每天重複使舌頭活塞的肌肉模式形成。' },
    ...
    'vent.cmp.row1-vent': { ko: '마우스필 압력 수준', en: 'Mouthfill pressure levels', ja: 'マウスフィル圧力水準', zh: 'Mouthfill 压力水平', tw: 'Mouthfill 壓力水準' },
    ...
    'vent.cmp.row3-diy': { ko: '없음 (입으로 직접)', en: 'None (mouthed directly)', ja: 'なし（直接口でくわえる）', zh: '无（直接用口）', tw: '無（直接用口）' },
    ...
    'vent.cmp.row4-vent': { ko: '여분 풍선 동봉', en: 'Multiple replacement balloons included', ja: '交換用バルーン多数同梱', zh: '附多个备用气球', tw: '附多個備用氣球' },
    'vent.cmp.row5': { ko: '센서 호환', en: 'Sensor compatibility', ja: 'センサー対応', zh: '传感器兼容', tw: '感測器相容' },
    'vent.cmp.row5-diy': { ko: '없음', en: 'None', ja: 'なし', zh: '无', tw: '無' },
    'vent.cmp.row5-vent': { ko: 'DiveChecker 센서와 호환', en: 'Pairs with the DiveChecker sensor', ja: 'DiveCheckerセンサーと併用可', zh: '与DiveChecker传感器兼容', tw: '與DiveChecker感測器相容' },
    ...
    'vent.bundle.do-desc': { ko: 'Vent로 충전·T-블록·텅피스톤 익히기', en: 'Drill charge → T-block → tongue piston with Vent', ja: 'Ventでチャージ・Tブロック・タングピストンを習得', zh: '用Vent训练charge → T-block → 舌头活塞', tw: '用Vent訓練charge → T-block → 舌頭活塞' },
    ...
    'vent.bundle.fix-desc': { ko: '앱 점수로 동작 교정', en: 'Read the app\'s scores and correct your motion', ja: 'アプリが分析したスコアを見て動作を修正', zh: '查看应用分析的分数纠正动作', tw: '查看應用分析的分數糾正動作' },
    'vent.bundle.cta': { ko: 'DiveChecker 센서 보기', en: 'View DiveChecker sensor', ja: 'DiveCheckerセンサーを見る', zh: '查看DiveChecker传感器', tw: '查看DiveChecker感測器' },
    ...
    'vent.specs.balloon-pres-val': { ko: '마우스필 훈련에 맞춰 교정 (자세한 수치는 출시 시 공개)', en: 'Calibrated for Mouthfill training (full spec at launch)', ja: 'マウスフィル訓練に合わせて校正（詳細仕様はリリース時に公開）', zh: '按Mouthfill训练校准（详细数值发售时公布）', tw: '按Mouthfill訓練校準（詳細數值發售時公布）' },
    ...
    'vent.specs.audience-val': { ko: '딥 프리다이빙(25m+) 준비 중이거나 마우스필을 익히려는 분', en: 'Freedivers preparing for deep dives (25m+) or working on Mouthfill', ja: 'ディープフリーダイビング(25m+)準備中、またはマウスフィル習得中の方', zh: '准备深潜(25m+)或学习Mouthfill的潜水员', tw: '準備深潛(25m+)或學習Mouthfill的潛水員' },
    ...
    // ── DISCLAIMER ──
    // ── VENT CLOSING ──
    'vent.closing.title': { ko: '당신이 못 하는 게 아니에요.<br>그동안 <span class="gradient-text glow-text">다질 곳</span>이 없었을 뿐.', en: "It's not that you can't.<br>You just had <span class=\"gradient-text glow-text\">nowhere to drill it</span>.", ja: 'できないのではありません。<br><span class="gradient-text glow-text">練習する場所</span>がなかっただけです。', zh: '不是您做不到，<br>只是以前没有<span class="gradient-text glow-text">练习的地方</span>。', tw: '不是您做不到，<br>只是以前沒有<span class="gradient-text glow-text">練習的地方</span>。' },
    'vent.closing.sub': { ko: 'DiveChecker Vent와 함께라면, 다음 다이브가 달라집니다.', en: 'With DiveChecker Vent, your next dive starts changing today.', ja: 'DiveChecker Ventと共に、次のダイブが変わります。', zh: '与DiveChecker Vent一起，下次潜水将不同。', tw: '與DiveChecker Vent一起，下次潛水將不同。' },
