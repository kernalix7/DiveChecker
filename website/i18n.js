...
const translations = {
    ...
    'faq.a10': { ko: 'USB-C 케이블로 연결 → 앱 실행 → 캘리브레이션 → 센서에 코를 대고 이퀄라이징. 3단계로 바로 시작할 수 있습니다. 자세한 내용은 <a href="guide.html">사용 가이드</a>를 참고하세요.', en: 'Connect via USB-C → Launch app → Calibrate → Place sensor on nose and equalize. Start in 3 simple steps. See the <a href="guide.html">User Guide</a> for details.', ja: 'USB-Cケーブルで接続 → アプリ起動 → キャリブレーション → センサーに鼻を当ててイコライジング。3ステップですぐに始められます。詳しくは<a href="guide.html">ユーザーガイド</a>をご覧ください。', zh: '通过USB-C线连接 → 启动应用 → 校准 → 将传感器放在鼻子上进行均压。3个简单步骤即可开始。详情请参阅<a href="guide.html">使用指南</a>。', tw: '通過USB-C線連接 → 啟動應用 → 校準 → 將傳感器放在鼻子上進行均壓。3個簡單步驟即可開始。詳情請參閱<a href="guide.html">使用指南</a>。' },
